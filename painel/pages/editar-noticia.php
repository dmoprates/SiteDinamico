<?php
	if(isset($_GET['id'])){
		$id = (int)$_GET['id'];
		$slide = Painel::select('site.noticias','id = ?',array($id));
	}else{
	...
	<form method="post" enctype="multipart/form-data">
		...
		<?php
			if(isset($_POST['acao'])){
				...
				$verifica = MySql::conectar()->prepare("SELECT `id` FROM `site.noticias` WHERE titulo = ? AND categoria_id = ? AND id != ?");
				...
				if($imagem['name'] != ''){
					...
					if(Painel::imagemValida($imagem)){
						...
						$arr = ['titulo'=>$nome,'data'=>date('Y-m-d'),'categoria_id'=>$_POST['categoria_id'],'conteudo'=>$conteudo,'capa'=>$imagem,'slug'=>$slug,'id'=>$id,'nome_tabela'=>'site.noticias'];
						Painel::update($arr);
						$slide = Painel::select('site.noticias','id = ?',array($id));
					...
					$imagem = $imagem_atual;
					$slug = Painel::generateSlug($nome);
					$arr = ['titulo'=>$nome,'categoria_id'=>$_POST['categoria_id'],'conteudo'=>$conteudo,'capa'=>$imagem,'slug'=>$slug,'id'=>$id,'nome_tabela'=>'site.noticias'];
					Painel::update($arr);
					$slide = Painel::select('site.noticias','id = ?',array($id));
		...
		<select name="categoria_id">
			<?php
				$categorias = Painel::selectAll('site.categorias');
