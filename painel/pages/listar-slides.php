<?php
    if (isset($_GET['excluir'])) {
        ...
        $selectImagem = MySql::conectar()->prepare("SELECT slide FROM `site.slides` WHERE id = ?");
        $selectImagem->execute(array($_GET['excluir']));
        $imagem = $selectImagem->fetch()['slide'];
        Painel::deleteFile($imagem);
        Painel::deletar('site.slides', $idExcluir);
        Painel::redirect(INCLUDE_PATH_PAINEL . 'listar-slides');
    } else if (isset($_GET['order']) && isset($_GET['id'])) {
        Painel::orderItem('site.slides', $_GET['order'], $_GET['id']);
    }
    $paginaAtual = isset($_GET['pagina']) ? (int) $_GET['pagina'] : 1;
    $porPagina = 4;
    $slides = Painel::selectAll('site.slides', ($paginaAtual - 1) * $porPagina, $porPagina);
?>
<div class="box-content">
    <h2><i class="fa-solid fa-images"></i> Slides Cadastrados</h2>
    ...
    <div class="paginacao">
        <?php
        $totalPaginas = ceil(count(Painel::selectAll('site.slides')) / $porPagina);
