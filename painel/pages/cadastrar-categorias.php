<div class="box-content">
    <h2> <i class="fa-solid fa-images"></i> Cadastrar Categoria</h2>
        ...
        <?php
			if(isset($_POST['acao'])){
				...
				if($nome == ''){
					...
					$verificar = MySql::conectar()->prepare("SELECT * FROM `site.categorias` WHERE nome = ?");
					$verificar->execute(array($_POST['nome']));
					if($verificar->rowCount() == 0){
					$slug = Painel::generateSlug($nome);
					$arr = ['nome'=>$nome,'slug'=>$slug,'order_id'=>'0','nome_tabela'=>'site.categorias'];
					Painel::insert($arr);
